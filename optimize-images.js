...
function optimizeImage(filePath) {
  try {
    ...
    if (ext === '.png') {
      ...
      execSync(`npx sharp-cli resize 1200 800 --input "${filePath}" --output "${optimizedPath.replace('.png', '.jpg')}" --quality ${QUALITY}`, { stdio: 'inherit' });
    } else {
      // Para JPEG
      execSync(`npx sharp-cli resize 1200 800 --input "${filePath}" --output "${optimizedPath}" --quality ${QUALITY}`, { stdio: 'inherit' });
    }
